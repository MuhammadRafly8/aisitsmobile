...
class ShipService extends ChangeNotifier {
    ...
    try {
      for (Polygon polygon in polygons) {
        if (isPointInPolygon(latLng, polygon.points)) {
          _handleShipInPolygon(shipName, polygon.color);
  ...
  void _handleShipInPolygon(String shipName, Color polygonColor) {
    try {
      final String message = polygonColor == Colors.red 
        ? "Kapal $shipName memasuki AREA TERLARANG!"
        : "Kapal $shipName memasuki AREA TERBATAS!";

      if (polygonColor == Colors.red) {
        _notificationService.addDangerMessage(message);
        LogUtils.info("Added danger notification for $shipName");
      } else if (polygonColor == Colors.blue) {
        _notificationService.addAlertMessage(message);
        LogUtils.info("Added alert notification for $shipName");
      }
      
      notifyListeners();
    } catch (e) {
      LogUtils.error('Error handling ship in polygon', e);
    }
  }
  @override
  void dispose() {
  ...
  Marker _createMarker(ShipData shipData, LatLng position) {
    return Marker(
      ...
      child: Transform.rotate(
        angle: _getRotationAngle(shipData),
  ...
  double _getRotationAngle(ShipData shipData) {
    // Gunakan true heading jika tersedia, jika tidak gunakan COG
    double angle = shipData.trueHeading > 0 ? shipData.trueHeading : shipData.cog;
    
    // Konversi ke radian dan sesuaikan orientasi
    return (angle - 180) * (pi / 180.0);
  }
